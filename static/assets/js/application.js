...
var Application = new function () {
	 ...
	 */
    function connectionDidLogIn(container, doNotAttackOnContainerClick, GUID) {
		...
		XPMobileSDK.getAllViews(function (items) {
            // console.log(items[0].Items[0].Items[0].Items)
            const Items = items[0].Items[0].Items[0].Items;
		    for (var i = 0; i < Items.length; i++) {
                if(GUID && GUID !== Items[i].Id) continue;
		        buildCameraElement(Items[i], container);
		    }
		});
    }
